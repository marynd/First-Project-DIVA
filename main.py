...
def init(path, format, scale, interpolation):
    images = sorted(glob.glob(args.path+"*{}".format(format)))
    for img in tqdm(images):
        images_resized = []
        img_resized = utils.rescale(img, format, scale=scale, interpolation=interpolation)
        ...
        images_resized[0].save(img+"_resized.gif", save_all=True, format="GIF", duration=100, loop=0)
